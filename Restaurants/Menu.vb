...
Public Class menu
    ...
    Dim conn As New MySqlConnection("server = localhost; user id = root; password = ; database = restaurantsystem;")
    ...
    Private Sub menu_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        reload("SELECT `ItemID`, `Description`, `Category`, `Price` FROM `menu`", DataGridView1)
        Try
            Dim sql As String
            ...
            sql = "SELECT `ItemID`, `Description`, `Category`, `Price` FROM `menu`"
    ...
    Private Sub Add_Button_Click(sender As Object, e As EventArgs) Handles Add_Button.Click
        ...
        Try
            ...
            Else
                create("INSERT INTO menu (ItemID, Description, Category, Price) VALUES ('" & TextBox1.Text & "', '" & TextBox2.Text & "', '" & ComboBox1.SelectedItem & "', '" & TextBox3.Text & "')")
                reload("SELECT `ItemID`, `Description`, `Category`, `Price` FROM `menu`", DataGridView1)
            End If

            reload("SELECT * FROM menu", DataGridView1)
        ...
        Try
            ...
            Else
                ...
                If result = DialogResult.Yes Then
                    updateTable("UPDATE `menu` SET `ItemID`='" & TextBox1.Text & "',`Description`='" & TextBox2.Text & "',`Category`='" & ComboBox1.SelectedItem & "',`Price`='" & TextBox3.Text & "' WHERE `ItemID` = " & DataGridView1.CurrentRow.Cells(0).Value.ToString() & "")
                    reload("SELECT `ItemID`, `Description`, `Category`, `Price` FROM `menu`", DataGridView1)
                    clearText()
                End If
            End If
    ...
    Private Sub Delete_Button_Click(sender As Object, e As EventArgs) Handles Delete_Button.Click
        Try
            ...
            Else
                ...
                If result = DialogResult.Yes Then
                    delete("DELETE FROM `menu` WHERE `ItemID` = " & DataGridView1.CurrentRow.Cells(0).Value.ToString())
                    reload("SELECT `ItemID`, `Description`, `Category`, `Price` FROM `menu`", DataGridView1)
                    clearText()
                End If
            End If
    ...
    Private Sub Save_Button_Click(sender As Object, e As EventArgs) Handles Save_Button.Click
        ...
        If result = DialogResult.Yes Then
            ...
            Try
                ...
                For i = 0 To DataGridView1.Rows.Count - 1
                    create("INSERT INTO `menu`(`ItemID`, `Description`, `Category`, `Price`) VALUES ('" & ItemID(i).ToString & "','" & Description(i).ToString & "','" & Category(i).ToString & "','" & Price(i).ToString & "')")
                Next
